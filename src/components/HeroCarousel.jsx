...
const slides = [{
  image: "/assets/carousel-tv-repair.jpg",
    ...
    <section className="tt-carousel">
      <div className="tt-carousel-viewport" ref={emblaRef}>
        <div className="tt-carousel-track">
          {slides.map((slide, i) => <div key={i} className="tt-carousel-slide">
      ...
      {/* Arrows */}
      <button className="tt-carousel-arrow tt-carousel-arrow-prev" onClick={scrollPrev} aria-label="Previous slide">
        <ChevronLeft size={18} />
      </button>
      <button className="tt-carousel-arrow tt-carousel-arrow-next" onClick={scrollNext} aria-label="Next slide">
        <ChevronRight size={18} />
      </button>

      {/* Dots */}
      <div className="tt-carousel-dots">
        {slides.map((_, i) => <button key={i} className={`tt-carousel-dot ${i === selectedIndex ? "active" : ""}`} onClick={() => emblaApi?.scrollTo(i)} aria-label={`Go to slide ${i + 1}`} />)}
      </div>
