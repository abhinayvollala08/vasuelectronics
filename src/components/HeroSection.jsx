...
const Counters = () => {
  ...
  return <div style={{
    ...
    gap: 10
      ...
      {stats.map((s, i) => <div key={i} ref={s.ref} style={{
      padding: "18px 10px",
      textAlign: "center",
      borderRadius: 12,
...
/* ─── Service chips row ─────────────────────────────────────────── */
const ServiceChips = () => <div style={{
  display: "flex",
  gap: 7,
  flexWrap: "wrap"
}}>
    {CHIPS.map((c, i) => <motion.span key={i} initial={{
    opacity: 0,
    y: 10,
    scale: .9
  }} animate={{
    ...
    color: "var(--tx2)",
    cursor: "default",
    boxShadow: "0 1px 4px rgba(13,21,38,.05)"
  }}>
        <span style={{
      width: 7,
      height: 7,
...
const Illustration = ({
  dark
}) => <svg viewBox="0 0 400 360" fill="none" xmlns="http://www.w3.org/2000/svg" style={{
  width: "100%",
  height: "100%",
  display: "block"
}}>
    ...
    {/* Background glow */}
    <ellipse cx="200" cy="185" rx="175" ry="145" fill={dark ? "rgba(74,155,255,.055)" : "rgba(26,95,191,.045)"} />

    {/* ── TV ── */}
    <rect x="55" y="72" width="238" height="162" rx="13" fill={dark ? "#1A2035" : "#E8F1FD"} stroke={dark ? "#2A3550" : "#BADAFF"} strokeWidth="1.8" />
    <rect x="68" y="84" width="212" height="124" rx="6" fill={dark ? "#06090F" : "#0D1526"} />

    <g className="il-scr">
      {[0, 1, 2, 3, 4, 5, 6, 7].map(i => <rect key={i} x="68" y={84 + i * 16} width="212" height="7" fill={dark ? "#4A9BFF" : "#1A5FBF"} opacity={.022 + i % 3 * .016} />)}
      {/* Progress ring */}
      <circle cx="174" cy="146" r="34" fill={dark ? "rgba(74,155,255,.12)" : "rgba(26,95,191,.1)"} stroke={dark ? "#4A9BFF" : "#1A5FBF"} strokeWidth="1.3" />
      <circle cx="174" cy="146" r="34" stroke={dark ? "#4A9BFF" : "#1A5FBF"} strokeWidth="3.5" strokeLinecap="round" fill="none" className="il-prog" transform="rotate(-90 174 146)" />
      <polyline points="161,146 171,156 189,138" stroke={dark ? "#4ADE80" : "#15803D"} strokeWidth="3.8" strokeLinecap="round" strokeLinejoin="round" fill="none" className="il-chk" />
      <text x="174" y="198" textAnchor="middle" fontFamily="'DM Mono',monospace" fontSize="8.5" letterSpacing="2.5" fill={dark ? "#4A9BFF" : "#1A5FBF"} opacity=".65">REPAIRED</text>
    ...
    {/* Speaker dots */}
    {[0, 1, 2].map(r => [0, 1, 2, 3].map(c => <circle key={`${r}-${c}`} cx={246 + c * 4.5} cy={94 + r * 4.5} r="1.1" fill={dark ? "#4A9BFF" : "#1A5FBF"} opacity=".26" />))}

    {/* Stand */}
    <rect x="152" y="234" width="48" height="13" rx="4" fill={dark ? "#1A2035" : "#D9E1EE"} stroke={dark ? "#2A3550" : "#BADAFF"} strokeWidth="1.1" />
    <rect x="137" y="246" width="78" height="7" rx="3.5" fill={dark ? "#111727" : "#C3CEDF"} />

    {/* Power LED */}
    <circle cx="264" cy="226" r="4.5" fill={dark ? "#0E1C30" : "#C3CEDF"} stroke={dark ? "#193150" : "#BADAFF"} strokeWidth="1" />
    <circle cx="264" cy="226" r="2.3" fill={dark ? "#4ADE80" : "#15803D"} className="il-led" />

    {/* ── Wrench ── */}
    <g className="il-tool">
      <rect x="288" y="158" width="13" height="50" rx="6.5" fill={dark ? "#2A3550" : "#1A5FBF"} opacity=".85" transform="rotate(-18 300 195)" />
      <circle cx="297" cy="163" r="9" fill="none" stroke={dark ? "#4A9BFF" : "#60A5FA"} strokeWidth="2.8" transform="rotate(-18 300 195)" />
    </g>

    {/* Sparks */}
    <g className="il-sp1"><polygon points="322,140 325,134 328,140 325,146" fill={dark ? "#FBBF24" : "#F59E0B"} opacity=".9" /></g>
    <g className="il-sp2"><polygon points="313,132 315,127 317,132 315,137" fill={dark ? "#FDE68A" : "#FBBF24"} opacity=".8" /></g>
    <g className="il-sp3"><polygon points="332,152 334,147 336,152 334,157" fill={dark ? "#FBBF24" : "#F59E0B"} opacity=".7" /></g>
    ...
    <text x="354" y="100" textAnchor="middle" fontFamily="'DM Mono',monospace" fontSize="7.5" letterSpacing=".5" fill={dark ? "#788499" : "#5E7191"}>LIVE</text>

    {/* Floating chips */}
    {/* Same-Day chip */}
    <g style={{
    animation: "ilTool 3s ease-in-out infinite",
    transformOrigin: "86px 52px"
  }}>
      <rect x="26" y="40" width="120" height="26" rx="13" fill={dark ? "#0E1C30" : "#E8F1FD"} stroke={dark ? "#193150" : "#BADAFF"} strokeWidth="1.1" />
      <circle cx="44" cy="53" r="4" fill="#4ADE80" />
      <text x="54" y="57" fontFamily="'DM Sans',sans-serif" fontSize="10" fontWeight="700" fill={dark ? "#ECF2FC" : "#0D1526"}>Same-Day</text>
    </g>
    {/* Doorstep chip */}
    <g style={{
    animation: "ilTool 3.8s ease-in-out .6s infinite",
    transformOrigin: "316px 46px"
  }}>
      <rect x="258" y="34" width="112" height="26" rx="13" fill={dark ? "#0E1C30" : "#E8F1FD"} stroke={dark ? "#193150" : "#BADAFF"} strokeWidth="1.1" />
      <text x="272" y="50" fontFamily="'DM Sans',sans-serif" fontSize="12">📍</text>
      <text x="290" y="51" fontFamily="'DM Sans',sans-serif" fontSize="10" fontWeight="700" fill={dark ? "#ECF2FC" : "#0D1526"}>Doorstep</text>
    </g>
    {/* Warranty chip */}
    <g style={{
    animation: "ilTool 4.2s ease-in-out 1s infinite",
    transformOrigin: "84px 285px"
  }}>
      <rect x="24" y="272" width="120" height="26" rx="13" fill={dark ? "#0E1C30" : "#E8F1FD"} stroke={dark ? "#193150" : "#BADAFF"} strokeWidth="1.1" />
      <text x="38" y="288" fontFamily="'DM Sans',sans-serif" fontSize="12">🛡️</text>
      <text x="57" y="289" fontFamily="'DM Sans',sans-serif" fontSize="10" fontWeight="700" fill={dark ? "#ECF2FC" : "#0D1526"}>Warranty</text>
    </g>
    {/* 15+ Brands chip */}
    <g style={{
    animation: "ilTool 3.5s ease-in-out 1.4s infinite",
    transformOrigin: "316px 287px"
  }}>
      <rect x="256" y="274" width="118" height="26" rx="13" fill={dark ? "#0E1C30" : "#E8F1FD"} stroke={dark ? "#193150" : "#BADAFF"} strokeWidth="1.1" />
      <text x="270" y="290" fontFamily="'DM Sans',sans-serif" fontSize="12">⚡</text>
      <text x="288" y="291" fontFamily="'DM Sans',sans-serif" fontSize="10" fontWeight="700" fill={dark ? "#ECF2FC" : "#0D1526"}>15+ Brands</text>
    </g>

    {/* ── Mini AC unit (bottom left) ── */}
    <rect x="62" y="314" width="74" height="38" rx="8" fill={dark ? "#1A2035" : "#D9E1EE"} stroke={dark ? "#2A3550" : "#BADAFF"} strokeWidth="1.1" />
    <rect x="66" y="320" width="66" height="5" rx="2.5" fill={dark ? "#4A9BFF" : "#1A5FBF"} opacity=".38" />
    {[0, 1, 2, 3].map(i => <rect key={i} x={68 + i * 14} y="330" width="11" height="16" rx="2" fill={dark ? "#0E1C30" : "#C3CEDF"} stroke={dark ? "#193150" : "#BADAFF"} strokeWidth=".8" />)}
    <circle cx="99" cy="347" r="2.5" fill={dark ? "#4ADE80" : "#15803D"} className="il-led2" />
    {/* AC airflow */}
    {[0, 1, 2].map(i => <path key={i} d={`M ${70 + i * 18} 310 Q ${79 + i * 18} 303 ${88 + i * 18} 310`} stroke={dark ? "#4A9BFF" : "#1A5FBF"} strokeWidth="1.1" strokeLinecap="round" fill="none">
        <animate attributeName="opacity" values=".08;.5;.08" dur={`${1.1 + i * .28}s`} repeatCount="indefinite" />
      </path>)}

    {/* ── Mini fridge (bottom right) ── */}
    <rect x="232" y="310" width="62" height="48" rx="8" fill={dark ? "#1A2035" : "#D9E1EE"} stroke={dark ? "#2A3550" : "#BADAFF"} strokeWidth="1.1" />
    <line x1="232" y1="332" x2="294" y2="332" stroke={dark ? "#2A3550" : "#BADAFF"} strokeWidth="1.1" />
    <rect x="237" y="316" width="5" height="12" rx="2.5" fill={dark ? "#4A9BFF" : "#1A5FBF"} opacity=".55" />
    <rect x="237" y="337" width="5" height="16" rx="2.5" fill={dark ? "#4A9BFF" : "#1A5FBF"} opacity=".55" />
  </svg>;
...
export const HeroSection = () => {
  ...
  return <>
      ...
      <div className="hw" data-theme={dark ? "dark" : "light"}>
        ...
        borderRadius: "50%",
        background: "radial-gradient(circle,var(--blob1) 0%,transparent 66%)",
        pointerEvents: "none"
      }} />
        <div style={{
        position: "absolute",
        ...
        borderRadius: "50%",
        background: "radial-gradient(circle,var(--blob2) 0%,transparent 68%)",
        pointerEvents: "none"
      }} />
        ...
        zIndex: 1
          ...
          <div className="hw-grid-layout">
            ...
            <div>
              ...
              duration: .48,
              ease: [.22, 1, .36, 1]
            }} className="hw-pill hw-pill-el" style={{
              width: "fit-content"
            }}>
              ...
              {/* Headline */}
              <motion.h1 className="hw-serif" initial={{
              opacity: 0,
              y: 20
            }} animate={{
              opacity: 1,
              ...
              {/* Accent rule */}
              <motion.div className="hw-rule" initial={{
              width: 0,
              opacity: 0
            }} animate={{
              width: 52,
              ...
              maxWidth: 490
                ...
                color: "var(--accent)",
                fontWeight: 600
              }}>same day</span>, every single time.
              </motion.p>
              ...
              marginBottom: 26
                ...
                <Link href="/services" className="hw-btn-g">
              ...
              delay: .54
            }} className="hw-trust" style={{
              display: "flex",
              gap: 18,
              flexWrap: "wrap"
            }}>
                {["Genuine OEM Parts", "Same-Day Response", "Doorstep Service", "Free Diagnosis"].map((t, i) => <div key={i} className="hw-trust-item">
            ...
            {/* ══ RIGHT ══════════════════════════════════════ */}
            <motion.div className="hw-right" initial={{
            opacity: 0,
            x: 22
          }} animate={{
            opacity: 1,
              ...
              <div className="hw-panel">
                ...
                padding: 14
                  ...
                  <div className="hw-dispatch">
                    ...
                    borderRadius: 9,
                    textDecoration: "none",
                    boxShadow: "0 4px 14px rgba(26,95,191,.28)",
                    whiteSpace: "nowrap"
                  }}>
        ...
        padding: "10px 0"
          ...
          <div className="hw-tick" style={{
          whiteSpace: "nowrap",
          width: "fit-content"
        }}>
            {[...Array(4)].map((_, k) => <span key={k} className="hw-mono" style={{
            color: "var(--tx3)",
            fontSize: ".7rem",
            letterSpacing: ".1em",
            display: "flex",
            ...
            paddingRight: 28
                ...
                {["LED TV Repair", "AC Servicing", "Refrigerator Repair", "Washing Machine Fix", "Home Theatre Setup", "AMC Contracts", "Genuine Parts", "Doorstep Service"].map((item, j) => <span key={j} style={{
              display: "flex",
              alignItems: "center",
              gap: 8
            }}>
