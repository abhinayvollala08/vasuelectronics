...
import { Home, Phone, Tv, Wrench, Mail } from "lucide-react";
...
const BrokenTV = ({
  dark
}) => <svg viewBox="0 0 200 160" fill="none" xmlns="http://www.w3.org/2000/svg" style={{
  width: "100%",
  height: "100%"
}}>
    <style>{`
    ...
    <g className="tv-wrap">
      {/* Body shell */}
      <rect x="22" y="28" width="156" height="102" rx="10" fill={dark ? "#1A2035" : "#E8F1FD"} stroke={dark ? "#2A3550" : "#BADAFF"} strokeWidth="1.8" />

      {/* Screen area */}
      <rect x="32" y="36" width="136" height="76" rx="5" fill={dark ? "#060810" : "#0D1526"} />
      ...
      <g className="tv-screen">
        ...
        {Array.from({
        length: 14
      }).map((_, i) => <rect key={i} x="32" y={36 + i * 5.6} width="136" height="2" fill={dark ? "#4A9BFF" : "#1A5FBF"} opacity={0.03 + i % 4 * 0.025} />)}
        {/* "?" glyph */}
        <text x="100" y="83" textAnchor="middle" fontFamily="'DM Serif Display', Georgia, serif" fontSize="38" fill={dark ? "#4A9BFF" : "#6CA8FF"} opacity=".9">?</text>
        {/* Scanline sweep */}
      ...
      {/* Crack */}
      <polyline points="96,44 89,60 101,64 91,82" stroke={dark ? "#FF7070" : "#DC2626"} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" opacity=".75" />
      <line x1="101" y1="64" x2="113" y2="70" stroke={dark ? "#FF7070" : "#DC2626"} strokeWidth="1.1" strokeLinecap="round" opacity=".45" />

      {/* Speaker dots */}
      {[0, 1, 2].map(row => [0, 1, 2, 3].map(col => <circle key={`${row}-${col}`} cx={148 + col * 4.5} cy={46 + row * 4.5} r="1.1" fill={dark ? "#4A9BFF" : "#1A5FBF"} opacity=".28" />))}

      {/* Bottom bar */}
      <rect x="22" y="128" width="156" height="2" rx="1" fill={dark ? "#0E1320" : "#C3CEDF"} opacity=".6" />

      {/* Power LED */}
      <circle cx="42" cy="122" r="4" fill={dark ? "#0E1C30" : "#C3CEDF"} stroke={dark ? "#193150" : "#BADAFF"} strokeWidth="1" />
      <circle cx="42" cy="122" r="2" fill={dark ? "#4A9BFF" : "#1A5FBF"} className="led-blink" />

      {/* Knob */}
      <circle cx="158" cy="122" r="6" fill={dark ? "#0E1C30" : "#C3CEDF"} stroke={dark ? "#193150" : "#BADAFF"} strokeWidth="1" />
      <line x1="158" y1="117" x2="158" y2="120" stroke={dark ? "#4A9BFF" : "#1A5FBF"} strokeWidth="1.4" strokeLinecap="round" />

      {/* Legs */}
      <rect x="62" y="129" width="14" height="10" rx="3" fill={dark ? "#1A2035" : "#D9E1EE"} stroke={dark ? "#2A3550" : "#BADAFF"} strokeWidth="1" />
      <rect x="124" y="129" width="14" height="10" rx="3" fill={dark ? "#1A2035" : "#D9E1EE"} stroke={dark ? "#2A3550" : "#BADAFF"} strokeWidth="1" />
      <rect x="58" y="138" width="22" height="5" rx="2.5" fill={dark ? "#111727" : "#C3CEDF"} />
      <rect x="120" y="138" width="22" height="5" rx="2.5" fill={dark ? "#111727" : "#C3CEDF"} />
    </g>
    ...
    d: "1s",
    dur: "2s"
  }].map((s, i) => <circle key={i} cx={s.x} r={s.r} fill={dark ? "#FBBF24" : "#F59E0B"}>
  ...
  return <>
      ...
      <div className="tt404" data-theme={dark ? "dark" : "light"}>
        ...
        <motion.div className="tt404-card" {...fadeUp(0)}>
          ...
          {/* Heading */}
          <motion.h1 className="tt404-serif" {...fadeUp(0.1)} style={{
          ...
          zIndex: 1
            ...
            <Link href="/" className="tt404-btn-p"><Home size={14} /> Back to Home</Link>
          ...
          zIndex: 1
            ...
            <div className="tt404-links">
              {quickLinks.map((l, i) => <Link key={i} href={l.to} className="tt404-link">
        ...
        {/* Footer */}
        <motion.p {...fadeUp(0.34)} className="tt404-mono" style={{
        marginTop: 20,
        fontSize: ".67rem",
        color: "var(--tx4)",
        letterSpacing: ".08em",
